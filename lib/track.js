...
Track.recent = function (cb) {
  db.query('SELECT * FROM tracks ORDER BY created_at DESC LIMIT 12', function (err, results) {
    if (err) throw(err)
...
Track.prototype = {
  ...
  save: function (callback) {
    ...
    db.query("INSERT INTO tracks (id, title, artist_name, created_at) VALUES (?,?,?,?) ON DUPLICATE KEY UPDATE play_count = play_count + 1", [self.id(), self.title(), self.artistName(), new Date ()], function (err, results) {
